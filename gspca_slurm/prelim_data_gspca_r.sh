...
declare {setwd,xfile,yfile,npc,nfolds,groups,gparams,kernel,niter,trace,balance}
OPTS=$(getopt -u -o '' -a --longoptions 'setwd:,xfile:,yfile:,npc:,nfolds:,groups:,gparams:,kernel:,niter:,trace:,balance:' -n "$0" -- "$@")
...
Rscript ${setwd}prelim_data_gspca_param.R $setwd $xfile $yfile $npc $nfolds $groups $gparams $kernel $niter $trace $balance
exit 0
