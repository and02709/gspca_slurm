...
gspca.obj <- cv_partition_group(arg.group=paramgrid[index,],df.partition=df,npc=npc,n.folds=nfolds,groups=group.list,kernel=kernel,ind.names=ind.names,niter=niter,trace=trace)
fpath <- paste(setwd,"temp/cv_outputs/job_",index,".txt",sep="")
data.obj <- data.frame(job=index,fold=paramgrid[index,1],sparse=paramgrid[index,2],cv.metric=gspca.obj)
